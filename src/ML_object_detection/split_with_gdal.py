...
def split_geotiff(image_path, output_dir, tile_width, tile_height, overlap):
    ...
    for y in range(0, img_height, y_step):
        for x in range(0, img_width, x_step):
            ...
            output_path = os.path.join(output_dir, f"{os.path.splitext(os.path.basename(image_path))[0]}_tile_{tile_id:05d}.tif")
            gdal.Translate(
                output_path,
                ds,
                srcWin=[x, y, w, h],
    ...
    print(f"Created {tile_id} tiles from '{image_path}' in '{output_dir}'.")
